...
const ALLOWED_MODELS = new Set(['openrouter/free']);
...
export default async function handler(req, res) {
  ...
  if (!apiKey) return res.status(500).json({ error: 'Missing OpenRouter API key configuration' });

  const body = req.body;
  if (!body || typeof body !== 'object' || !ALLOWED_MODELS.has(body.model)) {
    return res.status(400).json({ error: 'Invalid OpenRouter request' });
  }
  ...
  try {
    ...
    if (body.stream && upstream.body) {
      return upstream.body.pipeTo(new WritableStream({
        ...
        abort(err) {
          console.error('OpenRouter stream aborted:', err);
          res.end();
        },
      }));
    }
